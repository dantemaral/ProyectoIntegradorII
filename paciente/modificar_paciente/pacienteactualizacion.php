...
public function validacion_cedula(){
	...
	<a class='btn-enviar'  href='javascript:history.back(-1);' title='Ir la página anterior'>Volver</a></div></form>";
		...
		public function buscar_paciente_para_modificar()
		{
			include 'conectar.php';
			...
			if($reg=mysql_fetch_array($registros))
			{
				...
				*/
                ...
                header("Location: registro.php?cedu=$cedulapac&nom=$nombrespac&apell=$apellidospac&fechanac=$fecha_nacimiento_pac&tel=$telefonopac&sex=$sexopac&gene=$generpac&direc=$direccionpac&corre=$correopac&pass=$contrasenapac&grupo=$grupopac");
			}
			else 
			 {
				 echo " <br><h1> </h1><br>
	...
	<a class='btn-enviar'  href='javascript:history.back(-1);' title='Ir la página anterior'>Volver</a></div></form>";
	 ...
	 public function registro_paciente_actualizado()
	    {
		   include 'conectar.php';
		   
		   
		   mysql_query("UPDATE personas SET NOMBRES='$this->nombres', APELLIDOS='$this->apellidos',
		               FECHA_NACIMIENTO ='$this->fecha_nacimiento',TELEFONO='$this->telefono',SEXO='$this->sexo',
					   GENERO = '$this->genero', DIRECCION='$this->direccion',CORREO='$this->correo', EDAD='$this->edad'
					   WHERE CEDULA ='$this->cedula'",$conexion)or die ("Problemas en el select".mysql_error());
		   
 		   mysql_query("UPDATE pacientes SET CONTRASENIA='$this->contrasena', GRUPO_PRIORITARIO='$this->grupo_prioritario'
		                WHERE CEDULA ='$this->cedula'",$conexion) or die ("Problemas en el select".mysql_error());
 ...
 if($paciente->validacion_cedula()) 
  $paciente->buscar_paciente_para_modificar();*/
    ...
    if ($paciente->validar_fecha())
     {
       ...
       $paciente->registro_paciente_actualizado();
	   echo " <br><h1> </h1><br>
	<form  class='form-register'>
    <h2 class='form__titulo'>Paciente Modificado Exitosamente!!! </h2>
    <div class='contenedor-inputs'>      
    <a class='btn-enviar'  href='..\index.php'>Salir</a>
	<a class='btn-enviar'  href='javascript:history.back(-1);' title='Ir la página anterior'>Volver</a></div></form>";
     }
