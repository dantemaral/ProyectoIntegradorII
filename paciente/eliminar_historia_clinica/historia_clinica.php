...
public function validacion_cedula(){
	...
	<a class='btn-enviar'  href='javascript:history.back(-1);' title='Ir la página anterior'>Volver</a></div></form>";
		...
		public function buscar_historia_clinica_para_eliminar()
		{
			include 'conectar.php';
			$registros = mysql_query("select historiaclinica.cedula, antecedentespatologicos,observaciones from historiaclinica where historiaclinica.cedula='$this->cedula'",
			...
			if($reg=mysql_fetch_array($registros))
			{
				echo " <br><h1> </h1><br>
	<form  class='form-register'>
    <h2 class='form__titulo'> ¿Está seguro que desea eliminar la Historia Clínica ? </h2>
    <div class='contenedor-inputs'>      

    <a class='btn-enviar' href='eliminar.php?cedu=$this->cedula'>Confirmar</a>
	<a class='btn-enviar'  href='javascript:history.back(-1);' title='Ir la página anterior'>Volver</a></div></form>";
...
/*				echo "Cédula:  ".$reg['cedula']."<br>";
				echo "Antecedentes Patológicos:  ".$reg['antecedentespatologicos']."<br>";
				echo "Oservaciones:  ".$reg['observaciones']."<br>"."<br>";
				echo "<a class='btn-enviar' href='eliminar.php?cedu=$this->cedula'>Confirmar</a>";
	...
	*/			
	...
	public function ingreso_historia_clinica_actualizada()
	{
		include 'conectar.php';
		mysql_query("UPDATE historiaclinica SET ANTECEDENTESPATOLOGICOS='$this->antecedentes', OBSERVACIONES = '$this->observaciones'
		 WHERE CEDULA = '$this->cedula'",$conexion)or die ("Problemas en el select:".mysql_error());
		 mysql_close($conexion);
     	   echo " <br><h1> </h1><br>
	<form  class='form-register'>
    <h2 class='form__titulo'> Historia Clínica Actualizada!!! </h2>
    <div class='contenedor-inputs'>      
    <a class='btn-enviar'  href='..\index.php'>Salir</a>
	<a class='btn-enviar'  href='javascript:history.back(-1);' title='Ir la página anterior'>Volver</a></div></form>";
...
$historia_clinica = new historia_clinica($_REQUEST['ced_buscar'],"","");
 if($historia_clinica->validacion_cedula()) 
  $historia_clinica->buscar_historia_clinica_para_eliminar();
 ...
 */
 ...
 if($historia_clinica->validacion_cedula()) 
  $historia_clinica->ingreso_historia_clinica_actualizada();
*/
?>
</body>
</html>